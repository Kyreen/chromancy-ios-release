...
const serverAi = read("server-ai.ts");
...
expect(Boolean(pkg.dependencies?.["google-auth-library"]), "google-auth-library must stay installed as a direct dependency for Vertex AI service-account auth.");
expect(server.includes("app.use(helmet("), "Server must apply Helmet security headers.");
expect(serverAi.includes("vertexai: true"), "Server AI client must support Vertex AI mode.");
expect(serverAi.includes("VERTEX_AI_CONFIG_REQUIRED"), "Server AI client must fail clearly when Vertex AI project/location config is missing.");
expect(serverAi.includes("GoogleAuth"), "Server AI client must use GoogleAuth for Vertex video/download authentication.");
...
if (envProduction) {
  ...
  expect(envProduction.includes("CHROMANCY_AI_PROVIDER=vertex"), "Production AI provider must remain Vertex AI.");
  expect(envProduction.includes("GOOGLE_GENAI_USE_VERTEXAI=true"), "Production Vertex AI SDK flag must remain enabled.");
  expect(envProduction.includes("GOOGLE_CLOUD_PROJECT="), "Production env template must include GOOGLE_CLOUD_PROJECT for Vertex AI.");
  expect(envProduction.includes("GOOGLE_CLOUD_LOCATION="), "Production env template must include GOOGLE_CLOUD_LOCATION for Vertex AI.");
