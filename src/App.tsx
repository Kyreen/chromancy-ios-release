...
export default function App() {
  ...
  useEffect(() => {
    ...
    const setViewportVars = () => {
      // On native iOS the on-screen keyboard overlays the WKWebView. If we bind
      // the app shell to visualViewport.height, the whole layout (including the
      // bottom navigation bar) collapses upward and leaves a black gap when the
      // keyboard opens. Use the stable layout-viewport height there so the shell
      // stays full height and the focused input simply scrolls into view inside
      // its own scroll container. The safe-area insets (env()) still keep content
    ...
    // WKWebView can come back from the background with a stale page scale
    // (visualViewport.scale != 1) even though user zoom is disabled — the UI
    // then looks "too zoomed" until something re-applies the viewport rules.
    // Re-writing the viewport meta's initial-scale forces WebKit to snap the
    // page scale back to 1. Only runs when a wrong scale is actually detected.
    const resetPageZoom = () => {
      const vv = window.visualViewport;
      if (!vv || Math.abs(vv.scale - 1) < 0.02) return;
      const meta = document.querySelector('meta[name="viewport"]');
      if (!meta) return;
      const content = meta.getAttribute("content") || "width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no, viewport-fit=cover";
      const nudged = content.includes("initial-scale=1.0")
        ? content.replace("initial-scale=1.0", "initial-scale=1.0001")
        : `${content}, initial-scale=1.0001`;
      meta.setAttribute("content", nudged);
      requestAnimationFrame(() => meta.setAttribute("content", content));
    };

    // Re-apply on app resume (switching away and back can leave the viewport
    // zoomed/mis-sized on iOS until we recompute). iOS can also report a stale
    // window height for a moment after resume ("app too high"), so re-check a
    // few times as the viewport settles instead of trusting the first read.
    ...
    // When the keyboard dismisses (input blur), WebKit occasionally leaves the
    // page scaled or the layout height stale — snap both back.
    ...
    document.addEventListener("visibilitychange", handleResume, { passive: true });
    window.addEventListener("pageshow", handleResume, { passive: true });
    window.addEventListener("focus", handleResume, { passive: true });
