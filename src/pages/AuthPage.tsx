...
import { createAccountWithEmail, loginWithApple, loginWithEmail, loginWithGoogle, sendForgotPasswordEmail } from "../lib/firebase";
  ...
  switch (code) {
    ...
    default:
      ...
      if (message.toLowerCase().includes("user needs to") || message.toLowerCase().includes("sign in on")) {
        return "Your Apple ID needs attention. Open the Settings app on your iPhone → Apple ID → Password & Security → make sure Apple ID is fully active, then try again.";
      }
          ...
          <div className="space-y-4 rounded-[2rem] border border-white/10 bg-white/5 p-5">
            ...
            {isNativeIos && (
              <button
                ...
                disabled={isBusy || isResettingPassword}
                className="w-full flex items-center justify-center gap-2 rounded-3xl border border-white/10 bg-white px-4 py-4 text-xs font-bold uppercase tracking-widest text-black transition hover:bg-white/90 disabled:opacity-60"
              >
