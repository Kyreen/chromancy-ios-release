...
const PREMIUM_TOOLS = [
  ...
  { id: "clothes", icon: Shirt, label: "Clothes Changer", instruction: "Replace only the subject's clothing with the requested outfit while preserving the face, identity, hair, body proportions, pose, and background exactly.", description: "Change outfit" },
...
// Carousel display order only — all pricing/trial/AI logic still reads FREE_TOOLS and
// PREMIUM_TOOLS, so tool behaviour and billing are untouched by this ordering.
const ALL_PHOTO_TOOLS = [...FREE_TOOLS, ...PREMIUM_TOOLS];

const PHOTO_TOOL_DISPLAY_ORDER = [
  "manual",
  "sharp",
  "pose",
  "clothes",
...
const PHOTO_TOOLS_IN_ORDER = PHOTO_TOOL_DISPLAY_ORDER
  .map((id) => ALL_PHOTO_TOOLS.find((tool) => tool.id === id))
  .filter((tool): tool is (typeof ALL_PHOTO_TOOLS)[number] => Boolean(tool));
...
// Some Android pickers (Google Photos, Drive, other content providers) hand back a File
// whose bytes are streamed lazily. Reading it through createImageBitmap/objectURL then
// fails with "Could not load this photo". Pulling the bytes into memory first and
// rebuilding a plain in-memory Blob makes those files behave like any local file.
async function materialiseFile(file: File): Promise<Blob> {
  try {
...
async function normaliseUploadedPhoto(input: File): Promise<string> {
  const file = await materialiseFile(input) as File;
  const targetMimeType = getPreferredUploadMimeType(input);
  const quality = targetMimeType === "image/jpeg" ? 0.96 : 0.98;
  // 4096px is a ~67MB RGBA canvas, which low/mid-range Android devices fail to allocate,
  // producing a blank or throwing canvas. 3000px keeps plenty of detail for every tool.
...
// iOS WKWebView accepts `ctx.filter` as a property but silently ignores it when drawing,
// so manual-edit colour adjustments never baked into the saved photo on iOS. We apply them
// per-pixel instead, which works on every platform.
function bakeManualColorAdjust(
  ctx: CanvasRenderingContext2D,
  width: number,
  height: number,
  opts: { brightness: number; contrast: number; saturation: number; grayscale: number; sepia: number; hueDeg: number },
) {
  const image = ctx.getImageData(0, 0, width, height);
  const d = image.data;
  const { brightness: b, contrast: c, saturation: s, grayscale: gray, sepia: sep } = opts;
  const hue = (opts.hueDeg * Math.PI) / 180;
  const cosH = Math.cos(hue);
  const sinH = Math.sin(hue);
  const hueActive = Math.abs(opts.hueDeg) > 0.01;
  // Standard SVG/CSS hue-rotate matrix coefficients.
  ...
  const m4 = 0.715 + cosH * 0.285 + sinH * 0.140;
  ...
  for (let i = 0; i < d.length; i += 4) {
    ...
    r = luma + (r - luma) * s; g = luma + (g - luma) * s; bl = luma + (bl - luma) * s;
    // grayscale
    if (gray > 0) {
      const lg = 0.2126 * r + 0.7152 * g + 0.0722 * bl;
      r += (lg - r) * gray; g += (lg - g) * gray; bl += (lg - bl) * gray;
    }
    // sepia
    if (sep > 0) {
      ...
      r += (sr - r) * sep; g += (sg - g) * sep; bl += (sb - bl) * sep;
    }
    // hue-rotate
    if (hueActive) {
      const nr = r * m0 + g * m1 + bl * m2;
      const ng = r * m3 + g * m4 + bl * m5;
      const nb = r * m6 + g * m7 + bl * m8;
      r = nr; g = ng; bl = nb;
    }
    d[i] = r < 0 ? 0 : r > 1 ? 255 : r * 255;
    d[i + 1] = g < 0 ? 0 : g > 1 ? 255 : g * 255;
    d[i + 2] = bl < 0 ? 0 : bl > 1 ? 255 : bl * 255;
  }
  ctx.putImageData(image, 0, 0);
}

// Downscale/upscale blur approximation — works on iOS where ctx.filter blur is a no-op.
function bakeManualBlur(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, blurPx: number) {
  if (blurPx <= 0.01) return;
  const w = canvas.width;
  const h = canvas.height;
  const scale = Math.max(0.04, 1 / (1 + blurPx * 1.4));
...
async function renderManualEdit(source: string, state: ManualEditState): Promise<string> {
  ...
  const filterString = `brightness(${brightness}%) contrast(${contrast}%) saturate(${saturation}%) blur(${blur}px) grayscale(${grayscale}%) sepia(${sepia}%) hue-rotate(${hueRotate}deg)`;

  const drawGeometry = (useFilter: boolean) => {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.save();
    if (useFilter) ctx.filter = filterString;
  ...
  // Draw crop + rotation unfiltered (geometry works everywhere), then bake the colour
  // adjustments per-pixel so they actually apply on iOS. Native ctx.filter is only used
  // as a fallback if pixel access is unavailable (e.g. a tainted canvas).
  drawGeometry(false);
  let baked = false;
  try {
    bakeManualColorAdjust(ctx, canvas.width, canvas.height, {
      brightness: Number(brightness) / 100,
      contrast: Number(contrast) / 100,
      saturation: Number(saturation) / 100,
      grayscale: Number(grayscale) / 100,
      sepia: Number(sepia) / 100,
      hueDeg: Number(hueRotate),
    });
    bakeManualBlur(ctx, canvas, Number(blur));
    baked = true;
  } catch {
    baked = false;
  }
...
export function PhotoEnhancer({ tier, onNavigate }: PhotoEnhancerProps) {
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      console.error(error);
      toast.error("Could not load this photo. Please try saving it to your device first, then upload again.");
    }
  };
  ...
  const handleEnhance = async (toolId?: string, sourceOverride?: string) => {
    ...
    if (!localOnlyTools.has(id)) {
      ...
      if (id === "clothes" && !clothesPrompt) {
        toast.error("Please describe the new outfit");
        return;
      }
    ...
    try {
      ...
      if (id === "outpaint") {
        ...
        result = await enhancePhoto(
          outpaintSource,
          `Extend Photo: The uploaded image has the original photo centered on a transparent expanded canvas. Fill ONLY the transparent empty canvas around the original photo by continuing the real scene outwards. Preserve the centered original photo, subject, face, body, clothing, product details, lighting, and composition exactly — never repaint or regenerate an existing pixel. Continue every surface, edge, line, and repeating pattern that runs to the frame boundary with correct perspective, and continue lighting gradients, shadows, and reflections naturally. Match the original grain, sharpness, depth of field, colour response, and vignetting exactly. If this is a mirror selfie or contains reflective surfaces, keep the mirror edge and reflected geometry coherent. There must be NO visible line, seam, band, halo, blur ring, tone step, or texture change where the original photo ends. Do not crop, zoom, change the subject, add new people or objects, or add text or watermarks. ${PHOTO_AI_FACE_PRESERVATION_RULE}`,
          undefined,
          // sceneMode triggers the server-side scene planner, which analyses geometry,
          // lighting, and optics so the extension has no visible seam.
          { sceneMode: "extend" as const, ...aiMeta },
        );
      } else if (id === "remove") {
        ...
        result = await enhancePhoto(
          sourceImage,
          `Change Background: Replace only the background with this user request: ${bgPrompt}. Keep the subject unchanged. Resolve the subject boundary at pixel level — individual hair strands and flyaways, glasses, jewellery, straps, fingers, gaps between arm and body, and held objects — with no hard cut-out edge, halo, dark fringe, or leftover pixels of the old background. Match the subject's original camera angle, height, and perspective so the subject sits IN the scene rather than pasted on it, and match the lighting direction, intensity, colour temperature, and contrast already falling on the subject, adding realistic contact shadows and subtle colour spill. Reproduce the original depth of field, grain, and sharpness so both layers look shot on the same camera. Preserve face, body, hair, skin tone, clothing, accessories, product details, and foreground edges exactly. ${PHOTO_AI_FACE_PRESERVATION_RULE} Do not extend the canvas, do not crop, and do not alter or move the subject. The result must look like a real photograph taken in that location, with no visible composite seam.`,
          undefined,
          // sceneMode + backgroundRequest trigger the server-side scene planner for
          // pixel-accurate subject segmentation and camera/light matching.
          { sceneMode: "replace" as const, backgroundRequest: bgPrompt, ...aiMeta },
        );
      } else if (id === "clothes") {
        result = await enhancePhoto(
          sourceImage,
          `Change Clothes: Replace ONLY the subject's clothing with this exact outfit request: ${clothesPrompt}. Detect the subject and every garment with extreme precision, including partially hidden garment regions behind arms, hair, bags, or props. Render the requested outfit with photorealistic fabric texture, true material behaviour, natural drape and folds, accurate seams, and correct fit for the subject's existing body, with lighting, shadows, and colour grading that match the original scene exactly. Keep the subject's face, identity, hair, skin tone, body shape and proportions, pose, hands, non-clothing accessories, and the entire background EXACTLY unchanged. Fit the outfit to the body — never change the body to fit the outfit. Remove every trace of the previous outfit cleanly, and reconstruct any newly exposed skin naturally to match the subject. Keep the result tasteful and non-explicit. ${PHOTO_AI_FACE_PRESERVATION_RULE}`,
          undefined,
          // outfitRequest triggers the server-side vision planner, which analyses the
          // photo and the request into a structured garment plan before rendering.
          { outfitRequest: clothesPrompt, ...aiMeta },
        );
      } else if (id === "pose") {
  ...
  const handleToolSelection = useCallback(async (toolId: string) => {
    ...
    if (toolId !== "manual" && toolId !== "pose" && toolId !== "change_bg" && toolId !== "vibe" && toolId !== "clothes") {
  ...
  const shouldShowScrollHint = selectedTool === "change_bg" || selectedTool === "vibe" || selectedTool === "pose" || selectedTool === "clothes";
  ...
  const previewMediaClass = isManualPanelVisible && showManualTray ? "h-full w-full object-contain" : "max-w-full max-h-[72vh] object-contain";
    ...
    <div className="h-full flex flex-col bg-black">
      ...
      <div className="flex-1 relative bg-neutral-900/50">
        <div ref={previewViewportRef} className="visible-scrollbar absolute inset-0 overflow-y-auto p-4 pr-2">
          <div className={cn("min-h-full flex flex-col items-center gap-6", isManualPanelVisible ? "justify-start pt-2 pb-10" : "justify-center pb-32")}>
          ...
          <AnimatePresence mode="wait">
            {!currentImage ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative w-full flex flex-col items-center gap-6 transition-all duration-300">
                    ...
                    <div className="mx-auto w-full max-w-md rounded-[1.75rem] border border-white/10 bg-black/84 backdrop-blur-xl shadow-2xl overflow-hidden">
                      ...
                      <div ref={manualTrayScrollRef} className="manual-scrollbar max-h-[48vh] overflow-y-auto p-4 pt-3 space-y-3">
                        <div className="sticky top-0 z-20 -mx-4 -mt-3 mb-2 flex items-center justify-between gap-3 bg-gradient-to-b from-black via-black/95 to-black/80 px-4 pb-3 pt-3 text-white/75 backdrop-blur-sm">
                          ...
                          <button onClick={closeManualTray} className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-[0.18em] hover:bg-white/10 transition-colors">
                            Close tray
                          </button>
                        </div>
                ...
                {selectedTool === "clothes" && (
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md space-y-4 px-4">
                    <input
                      type="text"
                      placeholder="Describe the new outfit (e.g. black suit, summer dress, swimsuit...)"
                      value={clothesPrompt}
                      onChange={(e) => setClothesPrompt(e.target.value)}
                      className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-sm focus:outline-none focus:border-white/30 transition-all"
                    />
                    <button
                      onClick={() => handleEnhance("clothes")}
                      disabled={!currentImage || isProcessing || !clothesPrompt}
                      className="w-full py-4 rounded-2xl bg-white text-black font-bold uppercase tracking-[0.2em] hover:bg-white/90 disabled:opacity-20 transition-all flex items-center justify-center gap-2"
                    >
                      {isProcessing ? <div className="w-4 h-4 border-2 border-black/20 border-t-black rounded-full animate-spin" /> : <Sparkles className="w-4 h-4" />}
      ...
      <div ref={bottomToolBarRef} className="safe-area-bottom bg-black/80 backdrop-blur-md border-t border-white/10 p-4">
        <div className="visible-horizontal-scrollbar flex gap-3 overflow-x-auto pb-2">
          <button
            ...
            className="flex-shrink-0 flex flex-col items-center justify-center gap-2 p-3 rounded-2xl border border-red-500/20 bg-red-500/5 text-red-400 min-w-[80px] hover:bg-red-500/10 transition-all"
          >
            <RotateCcw className="w-6 h-6" />
            <p className="text-[10px] font-bold uppercase leading-none">Replace</p>
          </button>

          {PHOTO_TOOLS_IN_ORDER.map((tool) => (
            <button
              key={tool.id}
