...
import { cancelActiveAiRequests, enhancePhoto, generateVideo, posePerfect, isUnsuitableInputError, getAiRequestErrorMessage, isAiGenerationCancelledError, requestAiSpendConfirmation, settleLocalAiFallback } from "../lib/gemini";
...
const PHOTO_AI_FACE_PRESERVATION_RULE = "CRITICAL FACE PRESERVATION RULE: Preserve the subject's exact face, facial identity, likeness, and facial geometry exactly. Do not change face shape, eyes, nose, lips, jawline, cheeks, expression anatomy, skin feature placement, or hairline. Do not beautify by morphing, face-swap, regenerate, or make the person look like someone else. Apply only the requested tool effect.";
...
const PREMIUM_TOOLS = [
  ...
  { id: "animate", icon: Play, label: "ANIMATE", instruction: "Detect the primary subject intelligently, lock onto the true subject boundaries, and animate only the strongest believable motion zones with premium realism, cleaner subject separation, smoother temporal consistency, and more professional movement quality while preserving the subject's exact identity, facial structure, body proportions, and scene integrity. Avoid whole-frame wobble, warped limbs, facial drift, or random motion in background regions that should stay stable.", isVideo: true, description: "Add motion" },
];
...
export function PhotoEnhancer({ tier, onNavigate }: PhotoEnhancerProps) {
  ...
  const getUnsuitableInputMessage = (toolId: string) => {
    switch (toolId) {
      case "animate":
        return "Animate could not get a clean motion path from this photo yet. Please try again with the same image or use a clearer image with a stronger subject.";
      case "outpaint":
  ...
  const handleEnhance = async (toolId?: string, sourceOverride?: string) => {
    ...
    const aiMeta = { toolId: UI_TOOL_TO_INTERNAL_ID[id], tier };
    ...
    try {
      ...
      } else if (tool && "isVideo" in tool && tool.isVideo) {
        let animateChargeSettled = false;
        try {
          result = await generateVideo(
            sourceImage,
            "Create a premium professional animation from this still image. Detect the true content type and primary subject intelligently. The result must animate clearly and confidently from the first second and must never look like a static still. Lead with believable primary-subject motion where possible, then reinforce it with premium camera drift, parallax, light movement, atmospheric motion, or highlight motion so the finished clip feels alive, polished, and intentional. Preserve identity, framing, text, and scene integrity exactly while avoiding wobble, morphing, warped limbs, facial drift, or random background motion.",
            aiMeta,
          );
          animateChargeSettled = true;
          const { hasMeaningfulVideoMotion } = await import("../lib/videoProcessor");
          const animatedEnough = await hasMeaningfulVideoMotion(result);
          if (!animatedEnough) {
            try { URL.revokeObjectURL(result); } catch {}
            throw new Error("AI_VIDEO_STATIC_OUTPUT");
          }
        } catch (videoError) {
          console.warn("Primary animate generation fell back to the local living-photo renderer.", videoError);
          const { animateStillImage } = await import("../lib/videoProcessor");
          const fallbackResult = await animateStillImage(sourceImage);
          if (!animateChargeSettled) {
            try {
              await settleLocalAiFallback(aiMeta);
              animateChargeSettled = true;
            } catch (settlementError: any) {
              const status = Number((settlementError as any)?.status || 0);
              if (status === 401 || status === 402 || status === 403) {
                throw settlementError;
              }
              console.warn("Animate fallback billing settlement failed after a successful local render. Returning the finished animation anyway.", settlementError);
            }
          }
            ...
            {!currentImage ? (
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative w-full flex flex-col items-center gap-6 transition-all duration-300">
                <div ref={previewStageRef} className={previewStageClass}>
                  ...
                  {isProcessing && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center gap-4 z-50">
                      ...
                      <p className="text-xs font-bold uppercase tracking-[0.2em] animate-pulse">Processing...</p>
                    </div>
                  )}
                ...
                {isManualPanelVisible && (
                  ...
                  >
                    <div className={cn("pointer-events-auto mx-auto w-full max-w-md rounded-[1.75rem] border border-white/10 bg-black/84 backdrop-blur-xl shadow-2xl overflow-hidden transition-all duration-300", showManualTray ? "h-[30vh] sm:h-[32vh]" : "h-12")}>
                      <button onClick={() => setShowManualTray((v) => !v)} className="flex h-12 w-full items-center justify-between gap-3 border-b border-white/10 bg-white/[0.06] px-4 text-white/75 transition-colors hover:bg-white/[0.09]">
                        ...
                        {showManualTray ? <ChevronDown className="w-4 h-4" /> : <ChevronUp className="w-4 h-4" />}
                      </button>
                      {showManualTray && (
                        <div ref={manualTrayScrollRef} className="manual-scrollbar h-[calc(30vh-3rem)] overflow-y-auto p-4 space-y-3 sm:h-[calc(32vh-3rem)]">
                          ...
                          <div className="grid grid-cols-1 gap-2.5">
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop left</span><input type="range" min={0} max={95} value={manualEdit.cropX} onChange={(e) => setCropEdge("cropX", Number(e.target.value))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop top</span><input type="range" min={0} max={95} value={manualEdit.cropY} onChange={(e) => setCropEdge("cropY", Number(e.target.value))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop width</span><input type="range" min={5} max={100 - manualEdit.cropX} value={manualEdit.cropWidth} onChange={(e) => setCropEdge("cropWidth", Number(e.target.value))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop height</span><input type="range" min={5} max={100 - manualEdit.cropY} value={manualEdit.cropHeight} onChange={(e) => setCropEdge("cropHeight", Number(e.target.value))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Brightness</span><input type="range" min={0} max={200} value={manualEdit.brightness} onChange={(e) => commitManualState((prev) => ({ ...prev, brightness: Number(e.target.value) }))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Contrast</span><input type="range" min={0} max={200} value={manualEdit.contrast} onChange={(e) => commitManualState((prev) => ({ ...prev, contrast: Number(e.target.value) }))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Saturation</span><input type="range" min={0} max={200} value={manualEdit.saturation} onChange={(e) => commitManualState((prev) => ({ ...prev, saturation: Number(e.target.value) }))} className="w-full" /></label>
                            <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Blur</span><input type="range" min={0} max={10} step={0.1} value={manualEdit.blur} onChange={(e) => commitManualState((prev) => ({ ...prev, blur: Number(e.target.value) }))} className="w-full" /></label>
                          </div>
