            ...
            <div className="mx-auto w-full max-w-md rounded-[1.75rem] border border-white/10 bg-black/84 backdrop-blur-xl shadow-2xl overflow-hidden">
              ...
              <div ref={manualTrayScrollRef} className="manual-scrollbar max-h-[48vh] overflow-y-auto p-4 pt-3 space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2.5">
                  <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop left</span><input type="range" min={0} max={95} value={manualEdit.cropX} onChange={(e) => commitManualState((prev) => ({ ...prev, cropX: Number(e.target.value) }))} className="w-full" /></label>
                  <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop top</span><input type="range" min={0} max={95} value={manualEdit.cropY} onChange={(e) => commitManualState((prev) => ({ ...prev, cropY: Number(e.target.value) }))} className="w-full" /></label>
                  <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop width</span><input type="range" min={5} max={100 - manualEdit.cropX} value={manualEdit.cropWidth} onChange={(e) => commitManualState((prev) => ({ ...prev, cropWidth: Number(e.target.value) }))} className="w-full" /></label>
                  <label className="space-y-2"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">Crop height</span><input type="range" min={5} max={100 - manualEdit.cropY} value={manualEdit.cropHeight} onChange={(e) => commitManualState((prev) => ({ ...prev, cropHeight: Number(e.target.value) }))} className="w-full" /></label>
