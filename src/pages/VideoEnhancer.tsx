...
import { exportVideo, isFFmpegSupported, processVideo, warmVideoProcessor } from "../lib/videoProcessor";
...
function FlashCamStudio({ open, onClose, onSave }: FlashCamStudioProps) {
  ...
  const handleRecordToggle = useCallback(async () => {
    ...
    try {
      ...
      // Make sure the mic track is actually on before recording. On iOS the track can
      // exist but be muted/disabled, which produces a silent recording.
      try { liveAudioTrack.enabled = true; } catch {}
...
export function VideoEnhancer({ tier = "free", onNavigate }: VideoEnhancerProps) {
  ...
  const prepareBeamRecordingForEditor = useCallback(async (videoUrl: string, fileName: string) => {
    ...
    // Converting to MP4 needs the WASM video engine, which cannot run on iOS below 16.4.
    // Attempting it there used to stall Beam Mode for a long time after recording, so
    // hand the recording straight to the editor instead — it plays fine as-is.
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    ...
    // Some Android pickers (Google Photos, Drive, other content providers) return a File
    // whose bytes are streamed lazily; an object URL built straight from it fails to
    // decode with "Unable to load video". Reading the bytes into memory first and
    // rebuilding a plain Blob makes those clips load like any local file.
    let source: Blob = nextFile;
    try {
      const buffer = await nextFile.arrayBuffer();
      if (buffer && buffer.byteLength) {
        source = new Blob([buffer], { type: nextFile.type || "video/mp4" });
      }
    } catch (error) {
      console.warn("Could not buffer the picked video; using it directly.", error);
    }
  ...
  // Display order (all video tools are free): Manual Edit, Beam Mode, Video Reverse,
  // 1-Tap Video Fix, Fix Lighting, Pro Look.
  const videoTools = useMemo(() => [
    ...
    { id: "reverse", icon: Rewind, label: "Video Reverse", description: "Play in reverse" },
