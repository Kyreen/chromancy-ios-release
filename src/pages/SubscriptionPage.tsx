...
import { getWalletTopUpStorePrices, purchaseWalletTopUp } from "../lib/billing";
...
export function SubscriptionPage({ user, profile, tier, onBack }: SubscriptionPageProps) {
  ...
  // Localized store prices per product id (App Review 2.1(b): the app must show the
  // store's own localized IAP price, e.g. "$2.99", not a hardcoded "R50 + VAT").
  const [storePrices, setStorePrices] = useState<Record<string, string>>({});

  useEffect(() => {
    let cancelled = false;
    getWalletTopUpStorePrices()
      .then((prices) => {
        if (!cancelled && prices && Object.keys(prices).length) setStorePrices(prices);
      })
      .catch(() => undefined);
    return () => {
      cancelled = true;
  ...
  // Re-fetch the wallet whenever the user returns to this page/app (e.g. after a
  // purchase or switching away and back), so the balance never stays stale at R0.
  useEffect(() => {
  ...
  const handleWalletTopUp = async (productId: string, amountZar: number) => {
    const storePrice = storePrices[productId];
    const agreed = window.confirm(
      storePrice
        ? `Buy ${formatMoney(amountZar)} for ${storePrice}? The credits are added to your Chromancy wallet after the purchase.`
        : `Buy ${formatMoney(amountZar)}? The store will show the final localized price before you confirm.`,
    );
    if (!agreed) return;
    ...
    try {
      ...
      const snapshot = hasUpdatedBalance(syncedSnapshot)
        ? syncedSnapshot!
        : await waitForUsageSnapshot(hasUpdatedBalance, { tier, timeoutMs: 25000, intervalMs: 600 });
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-5 space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <div className="text-xs text-white/45">Wallet credits</div>
              ...
              <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
                <div className="text-xs text-white/45">Bonus credits</div>
        ...
        <div className="space-y-3">
          ...
          {WALLET_TOP_UPS.map((topUp) => {
            // Show the store's localized price (what the user is actually charged);
            // the credits amount is what gets added to their wallet.
            const storePrice = storePrices[topUp.productId];
            return (
              <div key={topUp.productId} className="rounded-3xl border border-white/10 bg-white/5 p-5 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-sm font-bold">{formatMoney(topUp.zar)}</div>
                    <div className="mt-1 text-[11px] text-white/45">Adds {formatMoney(topUp.zar)} to your Chromancy wallet.</div>
                  </div>
                  {storePrice ? <div className="text-right text-sm font-bold">{storePrice}</div> : null}
                </div>
                <button onClick={() => handleWalletTopUp(topUp.productId, topUp.zar)} className="w-full p-3 rounded-2xl border border-white/10 bg-black/40 font-bold uppercase tracking-widest text-xs hover:bg-white/10 transition-all">
                  {storePrice ? `Buy ${formatMoney(topUp.zar)} for ${storePrice}` : `Buy ${formatMoney(topUp.zar)}`}
                </button>
                {!storePrice ? <p className="text-[10px] text-white/35">The store shows the exact localized price before you confirm.</p> : null}
              </div>
            );
          })}
