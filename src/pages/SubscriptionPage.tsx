...
export function SubscriptionPage({ user, profile, tier, onBack }: SubscriptionPageProps) {
  ...
  const handleWalletTopUp = async (productId: string, amountZar: number) => {
    ...
    const agreed = window.confirm(
      storePrice
        ? `Buy the ${formatMoney(amountZar)} wallet top-up for ${storePrice}? The credit is added to your Chromancy wallet after the purchase.`
        : `Buy the ${formatMoney(amountZar)} wallet top-up? The store will show the final localized price before you confirm.`,
    );
    if (!agreed) return;
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          {WALLET_TOP_UPS.map((topUp) => {
            ...
            // the ZAR amount is the wallet credit they receive.
            ...
            return (
              <div key={topUp.productId} className="rounded-3xl border border-white/10 bg-white/5 p-5 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-sm font-bold">Wallet Top-up — {formatMoney(topUp.zar)} credit</div>
                    <div className="mt-1 text-[11px] text-white/45">Adds {formatMoney(topUp.zar)} to your Chromancy wallet.</div>
                  </div>
                  <div className="text-right text-sm font-bold">{storePrice || `${formatMoney(topUp.zar)}*`}</div>
                </div>
                <button onClick={() => handleWalletTopUp(topUp.productId, topUp.zar)} className="w-full p-3 rounded-2xl border border-white/10 bg-black/40 font-bold uppercase tracking-widest text-xs hover:bg-white/10 transition-all">
                  {storePrice ? `Buy for ${storePrice}` : "Buy"}
                </button>
                {!storePrice ? <p className="text-[10px] text-white/35">*Estimated — the store shows the exact localized price before you confirm.</p> : null}
              </div>
            );
          })}
