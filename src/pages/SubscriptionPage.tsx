...
function getBillingErrorMessage(error: unknown, fallback: string) {
  ...
  if (lower.includes("already") && (lower.includes("owned") || lower.includes("purchased"))) {
    return "This wallet product is being treated as already owned by the store. Wallet top-ups must be configured as consumable/non-subscription products in Google Play/App Store Connect and RevenueCat, then synced before testing again.";
  }

  if (lower.includes("problem with the store") || lower.includes("store_problem") || lower.includes("item_unavailable") || lower.includes("not found")) {
    return "The store could not complete this purchase. Confirm this wallet product is active, approved, consumable, attached to the RevenueCat offering, and available to your tester account.";
  }

  return message || fallback;
}

export function SubscriptionPage({ user, profile, tier, onBack, initialView, onTierChange }: SubscriptionPageProps) {
  ...
  const handleSubscribe = async (planId: string) => {
    ...
    try {
      ...
      const resolvedTier = isSubscriberTier(nextTier) ? nextTier : isSubscriberTier(snapshot?.tier) ? snapshot.tier : "free";
      if (isSubscriberTier(resolvedTier)) {
        onTierChange?.(resolvedTier);
      }
