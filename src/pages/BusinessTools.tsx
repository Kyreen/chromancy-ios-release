...
const LEVEL_UP_FACE_AND_TEXT_GUARD =
  "Hard quality rules: preserve all faces, body shapes, hands, hairlines, logos, products, packaging, and important scene details exactly. Do not morph people, beautify by changing facial geometry, face-swap, change ethnicity, change age, change expression anatomy, distort hands, or alter identity. Do not add any new or extra people, faces, characters, hands, objects, products, images, or graphic elements that were not already in the uploaded image or explicitly requested by the user. Do not add any new words, fake letters, glyphs, watermarks, signatures, app names, labels, prices, contact details, or filler copy. If text is not explicitly supplied by the user or already present in the uploaded image, leave text out. Finish with a clean premium commercial result, not a generic AI effect.";
...
export function BusinessTools({ tier = "free", onNavigate }: BusinessToolsProps) {
  ...
  const handleProcess = async (toolId: LevelUpToolId) => {
    ...
    let runId = 0;
    const aiMeta = { toolId: UI_TOOL_TO_INTERNAL_ID[toolId], tier, sessionKey: String(mediaSessionId) };
              ...
              <div className="relative rounded-3xl overflow-hidden bg-white/5 border border-white/10 min-h-[360px] flex items-center justify-center">
                ...
                <AnimatePresence>
                  {prediction && !isProcessing && (
                    ...
                    >
                      <div className="w-full max-w-md bg-black/90 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-2xl relative flex flex-col max-h-full min-h-0 overflow-hidden">
                        <button
              ...
              {isProcessing && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/45 backdrop-blur-sm">
                  <div className="flex flex-col items-center gap-3 rounded-3xl border border-white/10 bg-black/80 px-6 py-5 shadow-2xl">
                    ...
                    <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-white/75">AI is working...</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="visible-horizontal-scrollbar flex gap-3 overflow-x-auto pb-2">
          ...
          {tools.map((tool) => (
            <button
              ...
              className={cn(
                "flex-shrink-0 flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all min-w-[86px]",
