...
async function captureSourceAudio(video: HTMLVideoElement): Promise<{
  ...
  try {
    ...
    if (!tracks.length) { try { await ctx.close(); } catch {} return none; }
    return {
      tracks,
      // A muted element feeds silence into the graph, so the caller must unmute it.
      requiresUnmuted: true,
      cleanup: () => { try { source.disconnect(); } catch {} try { void ctx.close(); } catch {} },
    };
  } catch {
    return none;
  }
...
async function fastProcessVideo(videoUrl: string, action: VideoAction, params: ManualVideoParams = {}, options: ProcessVideoOptions = {}): Promise<string> {
  ...
  const sourceAudio = await captureSourceAudio(video);
...
async function exportFreeWatermarkedVideo(videoUrl: string): Promise<Blob> {
  ...
  const exportAudio = await captureSourceAudio(video);
