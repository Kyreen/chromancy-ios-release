...
async function exportFreeWatermarkedVideo(videoUrl: string): Promise<Blob> {
  ...
  // If we couldn't pull an audio track from the source (iOS WKWebView has no
  // HTMLVideoElement.captureStream), this canvas re-record would come out silent.
  // Bail so exportVideo falls back to the FFmpeg watermark path, which keeps audio.
  if (!mixedTracks.some((track) => track.kind === "audio")) {
    ...
    throw new Error("FREE_EXPORT_AUDIO_FALLBACK");
  }
...
// Deterministic FFmpeg colour/clarity grade for the no-AI video tools. iOS WKWebView records
// an unchanged/frozen frame through canvas.captureStream + MediaRecorder, so the canvas tools
// silently produce no visible effect there. FFmpeg.wasm runs identically on every platform.
function buildToneFilter(action: VideoAction): string {
  if (action === "light") {
    // Fix Lighting: brighten shadows, lift contrast + colour, add clarity.
    return "eq=brightness=0.07:contrast=1.18:saturation=1.16:gamma=1.04,unsharp=5:5:0.5:5:5:0.0";
  }
  if (action === "pro") {
    // Pro Look: punchy contrast, strong clarity, controlled colour, subtle vignette.
    return "eq=brightness=0.03:contrast=1.26:saturation=1.12:gamma=0.98,unsharp=7:7:1.0:7:7:0.0,vignette=PI/5";
  }
  // 1-Tap Fix: balanced one-tap enhance.
  return "eq=brightness=0.05:contrast=1.14:saturation=1.20:gamma=1.02,unsharp=5:5:0.7:5:5:0.0";
}
...
export async function processVideo(videoUrl: string, action: string, params: any = {}, options: ProcessVideoOptions = {}): Promise<string> {
  if (action === "light" || action === "1tap" || action === "pro" || action === "manual") {
    const canvasSupported = !!(window as any).MediaRecorder && !!(HTMLCanvasElement.prototype as any).captureStream;

    // iOS WKWebView records a frozen/unchanged frame through the canvas pipeline, so the
    // tools appear to do nothing. Run the deterministic FFmpeg grade there (and wherever the
    // canvas pipeline is unavailable). "manual" keeps the canvas path (it needs live crop
    // geometry) but still falls back to FFmpeg if that path throws.
    if (action !== "manual" && (isIOSWebView() || !canvasSupported)) {
    ...
    try {
      ...
      checkAbort(options.signal);
      console.warn("Canvas video pipeline failed; using FFmpeg grade fallback.", error);
      const outputPath = await ffmpegToneVideo(videoUrl, action as VideoAction, options);
      return await validatePlayableVideoUrl(outputPath);
    }
  }
