...
async function exportFreeWatermarkedVideo(videoUrl: string): Promise<Blob> {
  ...
  if (typeof captureFn === "function") {
    try {
      sourceStream = captureFn.call(video) as MediaStream;
      sourceStream.getAudioTracks().forEach((track) => mixedTracks.push(track));
    } catch (captureError) {
      console.warn("Free export: source audio capture failed.", captureError);
    }
  }

  // On iOS the source has no HTMLVideoElement.captureStream, so the watermarked re-record
  // is video-only (silent). We still export it — a watermarked silent clip is far better
  // than a failed export. (Paid exports pass the original mp4 through with audio intact.)
  const outputStream = new MediaStream(mixedTracks);
