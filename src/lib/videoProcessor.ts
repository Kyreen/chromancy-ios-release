...
// The canvas pipeline records in real time, so the ceiling must comfortably exceed the
// longest supported clip (90s) plus encode/flush overhead — otherwise long edits are
// killed mid-record.
const VIDEO_ABSOLUTE_TIMEOUT_MS = 150000;
const MAX_FAST_EDIT_SECONDS = 90;
...
// FFmpeg's WASM core is compiled with SIMD (v128). Safari only gained WASM SIMD in
// iOS 16.4, so on older iPhones the engine can never compile. Detecting that up front —
// with a 30-byte probe module — means we skip the 31 MB load entirely instead of
// downloading it and failing, which previously made Beam Mode hang after recording.
let wasmSimdSupported: boolean | null = null;
function supportsWasmSimd(): boolean {
  if (wasmSimdSupported !== null) return wasmSimdSupported;
  try {
...
// Once the engine is known to be unusable on this device, never retry it. Retrying cost
// a multi-second (and on mobile data, multi-megabyte) stall on every single call.
let ffmpegUnavailableReason: string | null = null;

export function isFFmpegSupported() {
  return ffmpegUnavailableReason === null && supportsWasmSimd();
}

export async function getFFmpeg() {
  if (ffmpeg && (ffmpeg as any).__loaded) return ffmpeg;
  if (ffmpegUnavailableReason) throw new Error(ffmpegUnavailableReason);
  if (!supportsWasmSimd()) {
    ffmpegUnavailableReason = "This device's browser engine does not support the video engine (WebAssembly SIMD).";
  ...
  loadPromise = (async () => {
    ...
    // Remember the failure. Without this, every later call re-ran all three attempts —
    // including a 31 MB CDN download — which stalled Beam Mode and every export.
    loadPromise = null;
    ffmpeg = null;
    ffmpegUnavailableReason = `The video engine could not start on this device. ${lastError instanceof Error ? lastError.message : String(lastError || "")}`.trim();
    throw new Error(ffmpegUnavailableReason);
  })();
...
export async function warmVideoProcessor() {
  ...
  // Preload the FFmpeg core in the BACKGROUND as soon as the Video tab opens. The core is a
  // large WASM download, and previously it was only fetched on first use — so the very first
  // FFmpeg-backed tool (Video Reverse) paid the entire download + compile cost before any
  // processing even started. Warming it while the user is still picking a clip removes that
  // wait entirely. Fire-and-forget: never block or throw into the UI.
...
// iOS WKWebView will not decode <video> frames or run canvas.captureStream() on elements
// that are detached from the DOM — the recorder then produces no data and errors out
// ("Video processing failed"). Attaching them hidden (off-screen, near-zero opacity, but
// NOT display:none — which also breaks capture) keeps them in the render tree so frames
// flow. Returns a detach function to call on cleanup.
function attachHiddenForCapture(...nodes: Array<HTMLElement | null | undefined>): () => void {
...
// Grab the source clip's audio as a recordable MediaStreamTrack.
//
// HTMLVideoElement.captureStream() does not exist in the iOS WKWebView, which meant every
// processed or exported video on iOS came out SILENT. The Web Audio graph is supported
// there, so we fall back to routing the element through a MediaStreamAudioDestinationNode.
// The graph is deliberately NOT connected to ctx.destination, so nothing is audible while
// processing — the audio only goes to the recorder.
// iOS keeps every AudioContext suspended until a real user gesture, and resume() can hang
// forever if called outside one. We therefore keep ONE shared context for the whole app and
// unlock it on the first touch/click, so it is already running by the time a tool needs it.
...
export function primeAudioForRecording() {
...
if (typeof window !== "undefined") {
  const unlock = () => { primeAudioForRecording(); };
...
/**
 * Decode the clip's audio straight from its bytes and play it into a recordable stream.
 * This bypasses the media element's audio plumbing entirely, so it works even where
 * createMediaElementSource yields nothing. Returns a `start()` the caller fires when
 * recording begins, so picture and sound stay in sync.
 */
async function decodeSourceAudio(sourceUrl: string): Promise<{
  ...
  try {
    ...
    const raw = await (await fetch(sourceUrl)).arrayBuffer();
    ...
    node.buffer = decoded;
    node.connect(destination); // never connected to the speakers — silent to the user
    const tracks = destination.stream.getAudioTracks();
    if (!tracks.length) return null;
    return {
      tracks,
      start: () => { try { node.start(); } catch {} },
      cleanup: () => { try { node.stop(); } catch {} try { node.disconnect(); } catch {} },
    };
  } catch {
    return null;
  }
...
async function captureSourceAudio(video: HTMLVideoElement, sourceUrl?: string): Promise<{
  tracks: MediaStreamTrack[];
  requiresUnmuted: boolean;
  start?: () => void;
  cleanup: () => void;
}> {
  const none = { tracks: [] as MediaStreamTrack[], requiresUnmuted: false, cleanup: () => {} };
  ...
  if (typeof captureFn === "function") {
    try {
      ...
      if (tracks.length) {
        return { tracks, requiresUnmuted: false, cleanup: () => { try { stream.getTracks().forEach((t) => t.stop()); } catch {} } };
      }
    } catch {}
  }

  try {
    if ((video as any).__chromancyAudioBound) return none; // createMediaElementSource is one-shot
    const ctx = getSharedAudioContext();
    if (!ctx) return none;

    if (ctx.state === "suspended") {
      // Never await this unbounded — on iOS it can hang indefinitely, which froze the
      // whole pipeline (Beam Mode appeared to stop working after recording).
      await Promise.race([
        ctx.resume().catch(() => undefined),
        new Promise((resolve) => setTimeout(resolve, 400)),
      ]);
    }
    if (ctx.state !== "running") return none; // stay silent rather than stall

    const source = ctx.createMediaElementSource(video);
    const destination = ctx.createMediaStreamDestination();
    source.connect(destination);
    (video as any).__chromancyAudioBound = true;
    const tracks = destination.stream.getAudioTracks();
    if (!tracks.length) return none;
    return {
      tracks,
      // A muted element feeds silence into the graph, so the caller must unmute it.
      requiresUnmuted: true,
      // The shared context is intentionally NOT closed — closing it would permanently
      // silence the element, and it is reused by later edits.
      cleanup: () => { try { source.disconnect(); } catch {} },
    };
  } catch {
    // Element routing failed — fall through to decoding the file's audio directly.
  }

  if (sourceUrl) {
    const decodedAudio = await decodeSourceAudio(sourceUrl);
    if (decodedAudio) {
      return {
        tracks: decodedAudio.tracks,
        requiresUnmuted: false,
        start: decodedAudio.start,
        cleanup: decodedAudio.cleanup,
      };
    }
  }
  return none;
...
// --- Non-AI video tone engine (Fix Lighting / Pro Look) --------------------
// ctx.filter is fast but some WebViews silently no-op or throw on it. We try
// ctx.filter and fall back to per-pixel math so the brightness/contrast/colour
// change ALWAYS renders. No AI — pure local pixel processing.
function applyVideoBaseTone(
  ctx: CanvasRenderingContext2D,
  sourceCanvas: HTMLCanvasElement,
  width: number,
  height: number,
  ...
  // Apply the tone via pixel math FIRST. iOS WKWebView accepts `ctx.filter` as a
  // property (so a feature check passes) but often no-ops the actual filtering during
  // drawImage — which previously made Fix Lighting / Pro Look / 1-Tap produce no visible
  // change on iOS. Pixel manipulation works on every device, so we lead with it.
  ctx.drawImage(sourceCanvas, 0, 0, width, height);
  try {
    const image = ctx.getImageData(0, 0, width, height);
    const d = image.data;
    for (let i = 0; i < d.length; i += 4) {
      let r = (d[i] / 255) * brightness;
      let g = (d[i + 1] / 255) * brightness;
      let b = (d[i + 2] / 255) * brightness;
    ...
    // getImageData can throw on a tainted canvas; fall through to the ctx.filter path.
  }

  try {
    ctx.save();
    ctx.filter = `brightness(${brightness.toFixed(3)}) contrast(${contrast.toFixed(3)}) saturate(${saturate.toFixed(3)})`;
    ctx.drawImage(sourceCanvas, 0, 0, width, height);
    ctx.restore();
  } catch {
...
function safeFrameStep(step: () => void) {
  try {
    step();
  } catch {
    // Best-effort polish pass; the base tone is already applied so the tool still
    // visibly improves the frame even if one pass is unsupported on this device.
  }
}
...
function drawProcessedFrame(
  ...
  if (action === "light") {
    // Fix Lighting (no AI): exposure is adaptive — it brightens dark footage and
    // lifts contrast + colour on brighter footage. Stronger than before and
    // guaranteed to render via the reliable tone engine.
    applyVideoBaseTone(ctx, sourceCanvas, width, height, exposure.brightness + 0.05, exposure.contrast + 0.12, exposure.saturate + 0.1);
    safeFrameStep(() => applyExposureFinish(ctx, width, height, exposure));
    safeFrameStep(() => applyGlobalPolish(ctx, width, height, 0.18));
    safeFrameStep(() => drawDetailBoost(ctx, sourceCanvas, width, height, 0.18));
    safeFrameStep(() => drawSharpenPass(ctx, sourceCanvas, width, height, 0.16));
    return;
  }

  if (action === "1tap") {
    // 1-Tap Fix (no AI): guaranteed visible base tone via the reliable pixel engine,
    // with the premium passes layered on best-effort so a single unsupported pass on
    // iOS can't abort the whole frame (which previously left the video unchanged).
    applyVideoBaseTone(ctx, sourceCanvas, width, height, exposure.brightness + 0.05, exposure.contrast + 0.08, exposure.saturate + 0.1);
    safeFrameStep(() => applyExposureFinish(ctx, width, height, exposure));
    safeFrameStep(() => applyPremiumNoiseControl(ctx, sourceCanvas, workCanvas, downsampleCanvas, width, height, 0.52));
    safeFrameStep(() => applySoftSkinFinish(ctx, sourceCanvas, workCanvas, width, height, 0.62));
    safeFrameStep(() => applyOneTapPolish(ctx, width, height));
    safeFrameStep(() => applyGlobalPolish(ctx, width, height, 0.46));
    safeFrameStep(() => drawDetailBoost(ctx, sourceCanvas, width, height, 0.22));
    safeFrameStep(() => drawSharpenPass(ctx, sourceCanvas, width, height, 0.12));
    return;
  }
  ...
  if (action === "pro") {
    // Pro Look (no AI): clean, sharp, professional-camera look — stronger clarity,
    // contrast and controlled colour, plus a subtle vignette. Base tone is
    // guaranteed; the premium passes are best-effort on top.
    applyVideoBaseTone(ctx, sourceCanvas, width, height, exposure.brightness + 0.02, exposure.contrast + 0.16, exposure.saturate + 0.08);
    safeFrameStep(() => applyExposureFinish(ctx, width, height, exposure));
    safeFrameStep(() => applyPremiumNoiseControl(ctx, sourceCanvas, workCanvas, downsampleCanvas, width, height, 0.56));
    safeFrameStep(() => applySoftSkinFinish(ctx, sourceCanvas, workCanvas, width, height, 0.34));
    safeFrameStep(() => applyLighting(ctx, width, height));
    safeFrameStep(() => applyProLook(ctx, width, height));
    safeFrameStep(() => applyGlobalPolish(ctx, width, height, 0.78));
    safeFrameStep(() => drawDetailBoost(ctx, sourceCanvas, width, height, 0.46));
    safeFrameStep(() => drawSharpenPass(ctx, sourceCanvas, width, height, 0.3));
...
async function fastProcessVideo(videoUrl: string, action: VideoAction, params: ManualVideoParams = {}, options: ProcessVideoOptions = {}): Promise<string> {
  ...
  // Attach the video + output canvas to the DOM (hidden) so iOS actually renders frames
  // and captureStream works. Detached elements silently fail on iOS WKWebView.
  const detachCapture = attachHiddenForCapture(video, canvas);

  const canvasStream = canvas.captureStream(TARGET_FPS);
  let sourceStream: MediaStream | null = null;
  const mixedTracks: MediaStreamTrack[] = [...canvasStream.getVideoTracks()];

  // Keep the source clip's audio. On iOS this uses the Web Audio fallback, because
  // HTMLVideoElement.captureStream() does not exist there — which is why every processed
  // and exported video previously came out silent on iPhone.
  const sourceAudio = await captureSourceAudio(video, videoUrl);
  for (const track of sourceAudio.tracks) mixedTracks.push(track);
  if (sourceAudio.requiresUnmuted) {
    // A muted element feeds silence into the Web Audio graph. Nothing is audible to the
    // user because the graph is never connected to the speakers.
    video.muted = false;
  ...
  try {
    ...
    // Unmuting for audio capture can trip autoplay policy on some devices. Losing the
    // audio is far better than failing the whole edit, so re-mute and try once more.
    let recovered = false;
    if (sourceAudio.requiresUnmuted) {
      try {
        video.muted = true;
...
async function convertBlobToMp4(blob: Blob, mode: "free" | "unlock" | "pro" = "pro"): Promise<Blob> {
  ...
  try {
    ...
    // Must comfortably cover a full-length (90s) clip being transcoded in WASM.
    const exitCode = await withTimeout(ffmpeg.exec(command), 240000, "Video export conversion timed out.");
    if (exitCode !== 0) {
...
async function exportFreeWatermarkedVideo(videoUrl: string): Promise<Blob> {
  ...
  // Attach video + canvas to the DOM (hidden) so iOS renders frames and captureStream
  // works — detached elements silently fail on iOS WKWebView.
  const detachCapture = attachHiddenForCapture(video, canvas);
  ...
  // Keep the audio on the watermarked re-record. iOS has no HTMLVideoElement.captureStream,
  // so captureSourceAudio falls back to a Web Audio graph there — previously this export
  // was always silent on iPhone.
  const exportAudio = await captureSourceAudio(video, videoUrl);
  for (const track of exportAudio.tracks) mixedTracks.push(track);
  if (exportAudio.requiresUnmuted) {
    video.muted = false;
    video.defaultMuted = false;
    video.removeAttribute("muted");
    video.volume = 1;
  ...
  try {
    ...
    // Unmuting for audio capture can trip autoplay policy. A silent export beats a
    // failed one, so re-mute and retry before giving up.
    let recovered = false;
    if (exportAudio.requiresUnmuted) {
      try {
        video.muted = true;
    ...
    if (!recovered) fail(error);
  }

  return await stopPromise;
}

// Deterministic FFmpeg colour grade — used only as a fallback if the canvas pipeline throws.
function buildToneFilter(action: VideoAction): string {
  // Use only the portable `eq` filter (brightness/contrast/saturation/gamma). unsharp and
  // vignette are not guaranteed to be compiled into the ffmpeg.wasm core build, and a
  // missing filter fails the whole command — eq is core and always present.
  if (action === "light") {
    return "eq=brightness=0.07:contrast=1.18:saturation=1.16:gamma=1.04";
  }
  if (action === "pro") {
    return "eq=brightness=0.03:contrast=1.26:saturation=1.14:gamma=0.98";
  }
  return "eq=brightness=0.05:contrast=1.14:saturation=1.20:gamma=1.02";
}

async function ffmpegToneVideo(videoUrl: string, action: VideoAction, options: ProcessVideoOptions = {}): Promise<string> {
  checkAbort(options.signal);
  const ffmpeg = await getFFmpeg();
  const { fetchFile } = await loadModules();
  const inputName = `tone_in_${Date.now()}.mp4`;
  const outputName = `tone_out_${Date.now()}.mp4`;
  try {
    checkAbort(options.signal);
    await ffmpeg.writeFile(inputName, await fetchFile(videoUrl));
    const command = [
      "-i", inputName,
      "-vf", buildToneFilter(action),
      "-c:v", "libx264", "-preset", "veryfast", "-crf", "23", "-pix_fmt", "yuv420p",
      "-c:a", "aac", "-b:a", "128k",
      "-movflags", "+faststart",
      outputName,
    ];
    const exitCode = await withTimeout(ffmpeg.exec(command), 180000, "Video enhancement timed out.");
    checkAbort(options.signal);
    if (exitCode !== 0) throw new Error(`FFmpeg video grade failed with code ${exitCode}`);
    const data = await ffmpeg.readFile(outputName);
...
// VIDEO REVERSE — local FFmpeg only, no AI. FFmpeg's `reverse` filter buffers every
// decoded frame in memory, so long or very large clips can exhaust the WASM heap on
// mobile. We cap the duration and downscale oversized footage before reversing.
const MAX_REVERSE_SECONDS = 90;
const MAX_REVERSE_DIMENSION = 720;
// The `reverse` filter holds every decoded frame of whatever it is given, so a 90s clip
// in one pass would need several GB and blow the WASM heap. Anything longer than the
// single-pass ceiling is therefore cut into chunks, each chunk reversed on its own, and
// the chunks concatenated back in reverse order — which caps peak memory at one chunk
// regardless of how long the clip is.
const REVERSE_SINGLE_PASS_MAX_SECONDS = 12;
const REVERSE_SEGMENT_SECONDS = 8;

async function getVideoMetadata(videoUrl: string, options: ProcessVideoOptions = {}) {
  const video = await loadVideo(videoUrl, options);
...
async function ffmpegReverseVideo(videoUrl: string, options: ProcessVideoOptions = {}): Promise<string> {
  checkAbort(options.signal);

  const { duration, width, height } = await getVideoMetadata(videoUrl, options);
  if (duration && duration > MAX_REVERSE_SECONDS) {
    throw new Error(`Video Reverse supports clips up to ${MAX_REVERSE_SECONDS} seconds. Please trim this video and try again.`);
  }

  // Downscale only when the source exceeds the safe dimension, preserving aspect ratio
  // and keeping both sides even (required by libx264 / yuv420p).
  const longestSide = Math.max(width, height);
  const scaleFilter = longestSide > MAX_REVERSE_DIMENSION
    ? (width >= height
        ? `scale=${MAX_REVERSE_DIMENSION}:-2,`
        : `scale=-2:${MAX_REVERSE_DIMENSION},`)
    : "";

  const ffmpeg = await getFFmpeg();
  const { fetchFile } = await loadModules();
  const stamp = Date.now();
  const inputName = `rev_in_${stamp}.mp4`;
  const outputName = `rev_out_${stamp}.mp4`;
  // Every intermediate file we create, so the virtual FS is always cleaned up.
  const scratchFiles: string[] = [inputName, outputName];

  const ENCODE_ARGS = [
    // ultrafast + zerolatency keeps the single-threaded WASM encoder as quick as possible;
    // crf 26 is visually fine at this resolution and encodes noticeably faster.
    "-c:v", "libx264", "-preset", "ultrafast", "-tune", "zerolatency", "-crf", "26", "-pix_fmt", "yuv420p",
  ];

  const exec = async (command: string[], timeoutMs: number, message: string) =>
    Number(await withTimeout(ffmpeg.exec(command), timeoutMs, message));

  // --- Single pass: short clips reverse in one go (fastest path) --------------
  const reverseSinglePass = async (withAudio: boolean) =>
    await exec([
      "-i", inputName,
      "-vf", `${scaleFilter}reverse`,
      ...(withAudio ? ["-af", "areverse"] : ["-an"]),
      ...ENCODE_ARGS,
      ...(withAudio ? ["-c:a", "aac", "-b:a", "96k"] : []),
      "-movflags", "+faststart",
      outputName,
    ], 90000, "Video reverse took too long. Please try a shorter clip.");

  // --- Segmented: long clips are chunked so peak memory stays bounded ---------
  const reverseSegmented = async (withAudio: boolean) => {
    const segPattern = `rev_seg_${stamp}_%03d.mp4`;

    // Pass 1 — normalise, downscale and cut into fixed-length chunks. Re-encoding here
    // (with a keyframe every second) guarantees the segmenter can cut cleanly, which a
    // stream copy cannot promise on arbitrary user footage.
    const segmentCode = await exec([
      "-i", inputName,
      ...(scaleFilter ? ["-vf", scaleFilter.replace(/,$/, "")] : []),
      ...ENCODE_ARGS,
      "-g", "30", "-keyint_min", "30", "-sc_threshold", "0",
      ...(withAudio ? ["-c:a", "aac", "-b:a", "96k"] : ["-an"]),
      "-f", "segment",
      "-segment_time", String(REVERSE_SEGMENT_SECONDS),
      "-reset_timestamps", "1",
      segPattern,
    ], 180000, "Video reverse took too long. Please try a shorter clip.");
    if (segmentCode !== 0) throw new Error(`FFmpeg segmenting failed with code ${segmentCode}`);

    // Discover the chunks that were actually produced.
    const expected = Math.ceil((duration || MAX_REVERSE_SECONDS) / REVERSE_SEGMENT_SECONDS) + 2;
    const segments: string[] = [];
    for (let i = 0; i < expected; i++) {
      const name = `rev_seg_${stamp}_${String(i).padStart(3, "0")}.mp4`;
      try {
        await ffmpeg.readFile(name);
        segments.push(name);
        scratchFiles.push(name);
      } catch {
        break;
      }
    }
    if (!segments.length) throw new Error("Video reverse could not split this clip.");

    // Pass 2 — reverse each chunk individually. Only one chunk is ever in memory.
    const reversedSegments: string[] = [];
    for (const segment of segments) {
      checkAbort(options.signal);
      const reversedName = `rev_part_${stamp}_${reversedSegments.length}.mp4`;
      ...
      if (code !== 0) throw new Error(`FFmpeg segment reverse failed with code ${code}`);
      reversedSegments.push(reversedName);
      scratchFiles.push(reversedName);
    }

    // Pass 3 — concatenate the reversed chunks in reverse order. Stream copy, so this
    // is near-instant and adds no quality loss.
    const listName = `rev_list_${stamp}.txt`;
    scratchFiles.push(listName);
    const listBody = reversedSegments
      .slice()
      .reverse()
      .map((name) => `file '${name}'`)
      .join("\n");
  ...
  const useSegmented = !duration || duration > REVERSE_SINGLE_PASS_MAX_SECONDS;
  const runReverse = (withAudio: boolean) => (useSegmented ? reverseSegmented(withAudio) : reverseSinglePass(withAudio));
  ...
  try {
    ...
    let exitCode: number;
    try {
      exitCode = await runReverse(true);
    } catch (error) {
      checkAbort(options.signal);
      // Silent clips have no audio stream for `areverse` to consume — retry video-only.
      console.warn("Reverse with audio failed; retrying without audio.", error);
      exitCode = await runReverse(false);
    }

    if (exitCode !== 0) {
      checkAbort(options.signal);
      exitCode = await runReverse(false);
      if (exitCode !== 0) throw new Error(`FFmpeg video reverse failed with code ${exitCode}`);
    }
...
// ---------------------------------------------------------------------------
// VIDEO REVERSE — canvas fallback (no FFmpeg, no WebAssembly)
//
// @ffmpeg/core 0.12.x is compiled with WebAssembly SIMD (v128 locals). Safari only
// gained WASM SIMD in iOS 16.4, so on older iPhones the engine fails to COMPILE with
// "can't get Function local's type ... in function at index 10" and no FFmpeg-backed
// tool can run at all. This path uses the same canvas + MediaRecorder pipeline the
// other video tools already use successfully on iOS, so Video Reverse works on every
// device the app supports.
//
// Frames are captured forward (at accelerated playback, since audio is handled
// separately) into compressed JPEG blobs to keep memory bounded, then replayed in
// reverse in real time while recording. Audio is reversed with the Web Audio API.
// ---------------------------------------------------------------------------

const CANVAS_REVERSE_FPS = 12;
const CANVAS_REVERSE_MAX_WIDTH = 640;
// Kept modest on purpose: the proven canvas pipeline used by the other video tools
// plays at 1x, and high playback rates cause dropped/undecoded frames on iOS.
const CANVAS_REVERSE_CAPTURE_RATE = 2;

async function reverseAudioTrack(videoUrl: string, options: ProcessVideoOptions = {}) {
  // Best effort: a silent clip, an undecodable container, or a blocked AudioContext
  // must never fail the whole reverse — we simply produce a silent result.
  try {
    checkAbort(options.signal);
    ...
    const decoded = await ctx.decodeAudioData(raw.slice(0));
    if (!decoded || decoded.length === 0) { try { await ctx.close(); } catch {} return null; }
    ...
    return { ctx, node, stream: destination.stream };
  } catch (error) {
    console.warn("Reversed audio unavailable; producing a silent reverse.", error);
    return null;
  }
}

async function canvasReverseVideo(videoUrl: string, options: ProcessVideoOptions = {}): Promise<string> {
  if (!(window as any).MediaRecorder || !(HTMLCanvasElement.prototype as any).captureStream) {
    throw new Error("Video Reverse is not supported on this device.");
  }
  checkAbort(options.signal);
  ...
  const scale = srcW > CANVAS_REVERSE_MAX_WIDTH ? CANVAS_REVERSE_MAX_WIDTH / srcW : 1;
  const width = Math.max(2, Math.round(srcW * scale / 2) * 2);
  const height = Math.max(2, Math.round(srcH * scale / 2) * 2);

  const grabCanvas = createCanvas(width, height);
  const grabCtx = grabCanvas.getContext("2d", { alpha: false });
  const outCanvas = createCanvas(width, height);
  const outCtx = outCanvas.getContext("2d", { alpha: false });
  if (!grabCtx || !outCtx) throw new Error("Video Reverse could not prepare the canvas.");

  // iOS WKWebView will not decode <video> frames unless the element is in the document.
  const detach = attachHiddenForCapture(video, outCanvas);
  const frames: Blob[] = [];

  try {
    // ---- Phase 1: capture frames forward -----------------------------------
    video.muted = true;
    video.currentTime = 0;
    try { video.playbackRate = CANVAS_REVERSE_CAPTURE_RATE; } catch {}

    await video.play().catch(() => undefined);

    const interval = 1 / CANVAS_REVERSE_FPS;
    let nextSampleAt = 0;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const guard = window.setTimeout(() => fail(new Error("Video Reverse timed out while reading the clip.")), 180000);

      // Driven by a timer rather than requestAnimationFrame: we sample at a fixed rate,
      // so display refresh is irrelevant, and rAF is throttled to zero whenever the app
      // is backgrounded or not compositing — which would stall capture completely.
      const step = async () => {
        if (done) return;
        if (options.signal?.aborted) { window.clearTimeout(guard); return fail(new DOMException("Video processing cancelled.", "AbortError")); }
        try {
          if (video.currentTime + 0.0001 >= nextSampleAt) {
            grabCtx.drawImage(video, 0, 0, width, height);
            const blob = await new Promise<Blob | null>((res) => grabCanvas.toBlob((b) => res(b), "image/jpeg", 0.82));
            if (blob) frames.push(blob);
            // Never let the sampler fall behind the playhead on a slow device.
            nextSampleAt = Math.max(nextSampleAt + interval, video.currentTime);
          }
        } catch (error) {
      ...
      // Drive with BOTH rAF and a timer, whichever fires first, de-duplicated by a
      // shared token. rAF matches the pipeline the other video tools already use
      // successfully on iOS; the timer keeps capture alive if rAF is throttled
      // (backgrounded, or not compositing).
      let tick = 0;
      function schedule() {
        const mine = ++tick;
        const run = () => { if (mine !== tick || done) return; tick++; void step(); };
        requestAnimationFrame(run);
        window.setTimeout(run, 16);
      }

      video.onended = () => { window.clearTimeout(guard); finish(); };
      schedule();
    });

    try { video.pause(); } catch {}
    if (frames.length < 2) {
      // Include the counters so an on-device failure is diagnosable from the toast alone.
      throw new Error(`Video Reverse could not read this clip (frames: ${frames.length}, length: ${duration.toFixed(1)}s). Please try a different video.`);
    }

    // ---- Phase 2: reversed audio (best effort) -----------------------------
    const audio = await reverseAudioTrack(videoUrl, options);

    // ---- Phase 3: replay in reverse while recording ------------------------
    // captureStream(0) + requestFrame() pushes exactly one frame per draw instead of
    // relying on the compositor to sample the canvas — the compositor stalls whenever
    // the canvas is offscreen, which otherwise yields a near-empty recording.
    const videoTrackSupportsManualFrames = typeof (HTMLCanvasElement.prototype as any).captureStream === "function";
    let canvasStream: MediaStream = outCanvas.captureStream(0);
    let manualTrack: any = canvasStream.getVideoTracks()[0];
    if (!manualTrack || typeof manualTrack.requestFrame !== "function") {
      canvasStream = outCanvas.captureStream(CANVAS_REVERSE_FPS);
      manualTrack = null;
    }
    void videoTrackSupportsManualFrames;
    const tracks: MediaStreamTrack[] = [...canvasStream.getVideoTracks()];
    if (audio) for (const track of audio.stream.getAudioTracks()) tracks.push(track);
    const recorder = createFastVideoRecorder(new MediaStream(tracks), "1tap");

    const chunks: Blob[] = [];
    recorder.ondataavailable = (event) => { if (event.data && event.data.size) chunks.push(event.data); };
    const recorded = new Promise<Blob>((resolve, reject) => {
      recorder.onstop = () => {
        const blob = new Blob(chunks, { type: chunks[0]?.type || getPreferredRecorderMimeType() || "video/webm" });
        blob.size ? resolve(blob) : reject(new Error("Video Reverse produced an empty file."));
      };
      recorder.onerror = () => reject(new Error("Video Reverse recording failed."));
    });

    recorder.start(250);
    audio?.node.start();

    const frameMs = 1000 / CANVAS_REVERSE_FPS;
    for (let i = frames.length - 1; i >= 0; i--) {
      ...
      try { manualTrack?.requestFrame(); } catch {}
...
export async function processVideo(videoUrl: string, action: string, params: any = {}, options: ProcessVideoOptions = {}): Promise<string> {
  if (action === "reverse") {
    // Prefer FFmpeg where it runs (much faster). Fall back to the canvas pipeline on
    // devices whose WebAssembly engine cannot compile the SIMD build — notably iOS < 16.4.
    try {
      const outputPath = await ffmpegReverseVideo(videoUrl, options);
      return await validatePlayableVideoUrl(outputPath);
    } catch (error) {
      if (options.signal?.aborted) throw error;
      console.warn("FFmpeg reverse unavailable; using the canvas reverse pipeline.", error);
      (window as any).__chromancyReverseFallbackReason = String((error as any)?.message || error);
      const outputPath = await canvasReverseVideo(videoUrl, options);
      return await validatePlayableVideoUrl(outputPath);
    }
  }

  if (action === "light" || action === "1tap" || action === "pro" || action === "manual") {
    // Primary path everywhere is the canvas + MediaRecorder pipeline (hardware-accelerated,
    // mobile-viable). It now works on iOS because the video + canvas are attached to the DOM
    // during processing (see attachHiddenForCapture). If that path still throws on a device,
    // fall back to the FFmpeg grade (not for "manual", which needs live crop geometry).
    try {
      const outputPath = await fastProcessVideo(videoUrl, action as VideoAction, params, options);
      return await validatePlayableVideoUrl(outputPath);
    } catch (error) {
      if (action === "manual") throw error;
      checkAbort(options.signal);
      console.warn("Canvas video pipeline failed; trying FFmpeg grade fallback.", error);
      const outputPath = await ffmpegToneVideo(videoUrl, action as VideoAction, options);
      return await validatePlayableVideoUrl(outputPath);
    }
  }
  ...
  try {
    checkAbort(options.signal);
    await ffmpeg.writeFile(inputName, await fetchFile(videoUrl));
    const exitCode = await withTimeout(ffmpeg.exec(buildCommand(inputName, outputName)), 240000, "Video processing timed out before completion.");
    checkAbort(options.signal);
