...
// ctx.filter is fast but some iOS WebViews silently no-op or throw on it, which
// is why Fix Lighting / Pro Look could look like they did nothing on iPhone. We
// try ctx.filter and fall back to per-pixel math so the brightness/contrast/
// colour change ALWAYS renders. No AI — pure local pixel processing.
...
function drawProcessedFrameEnhanced(
...
function drawProcessedFrame(
  ...
  try {
    drawProcessedFrameEnhanced(ctx, sourceCanvas, workCanvas, maskCanvas, maskBlurCanvas, downsampleCanvas, action, faces, width, height, previousSkinMaskCanvas);
  } catch (error) {
    // Some iOS WKWebView versions throw inside an individual enhancement step
    // (for example ctx.filter or a pixel pass that Android handles fine). Rather
    // than failing the whole video tool — which is why only Manual Edit worked on
    // iOS — degrade gracefully to the unprocessed frame so the edit still produces
    // a valid video.
    console.warn("Video frame enhancement failed; using source frame instead.", error);
    try {
      ctx.clearRect(0, 0, width, height);
      ctx.drawImage(sourceCanvas, 0, 0, width, height);
    } catch {}
