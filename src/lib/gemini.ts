...
export interface AiRequestMeta {
  ...
  tier?: string;
  retryOfJobId?: string;
  /** Identifies the current file/edit session so a consecutive re-run of the same
   *  paid tool on the same file is detected as a half-price retry. */
  sessionKey?: string;
}
...
// Output is ~1K (1024px), so an input larger than ~1400px adds upload + decode time with
// no benefit. Trimmed for faster generation while staying well above output res (keeps face
// detail). Override with VITE_CHROMANCY_AI_SOURCE_MAX_DIMENSION / _MAX_BYTES.
const MAX_AI_SOURCE_DIMENSION = Number(import.meta.env.VITE_CHROMANCY_AI_SOURCE_MAX_DIMENSION || 1400);
const MAX_AI_SOURCE_BYTES = Number(import.meta.env.VITE_CHROMANCY_AI_SOURCE_MAX_BYTES || 1_400_000);
...
// --- Half-price retry detection -------------------------------------------
// When a user runs a paid tool and then immediately runs the SAME tool again on
// the SAME file, the second run is a "retry" and costs half price (retryZar).
// We track the last confirmed paid run per file/session; a match flags the meta
// with retryOfJobId so both the confirmation dialog and the server apply the
// half price. A new upload (new sessionKey) or a different tool resets it.
let lastPaidToolKey: string | null = null;

function getPaidToolKey(meta?: AiRequestMeta) {
  if (!meta?.toolId || !meta?.sessionKey) return null;
  return `${meta.toolId}::${meta.sessionKey}`;
}

// Premium AI tools never get the half-price retry — they stay full price.
const NO_RETRY_DISCOUNT_TOOLS = new Set(["create"]);

function detectConsecutiveRetry(meta?: AiRequestMeta) {
  if (!meta || meta.retryOfJobId) return;
  if (meta.toolId && NO_RETRY_DISCOUNT_TOOLS.has(meta.toolId)) return;
  const key = getPaidToolKey(meta);
  if (key && lastPaidToolKey === key) {
    meta.retryOfJobId = `retry:${meta.toolId}`;
  }
}

function rememberPaidToolRun(meta?: AiRequestMeta) {
  const key = getPaidToolKey(meta);
  if (key) lastPaidToolKey = key;
}

export function resetRetryTracking() {
  lastPaidToolKey = null;
}
...
function getAiSpendContext(meta?: AiRequestMeta) {
  ...
  const isRetry = !!meta?.retryOfJobId;
  const isSubscriber = isSubscriberTier(meta?.tier === "premium" ? "premium" : meta?.tier === "pro" ? "pro" : "free");
  const creditCost = getToolProCreditCost(tool.internalId, isRetry);
  const priceZar = getToolPriceZar(tool.internalId, isRetry);
  const fullPriceZar = getToolPriceZar(tool.internalId, false);
  const creditLabel = `${creditCost} AI Subscription Credit${creditCost === 1 ? "" : "s"}`;
  const walletLabel = `${formatZarForConfirm(priceZar)} from your wallet`;

  if (isRetry) {
    const creditNote = isSubscriber
      ? ` (or ${creditLabel} if you have subscription credits available)`
      : "";
    return {
      title: `Retry ${tool.displayName} — discounted`,
      message: `Repeat discount! Because you just used ${tool.displayName} on this file, this retry is reduced to only ${formatZarForConfirm(priceZar)} instead of ${formatZarForConfirm(fullPriceZar)}${creditNote}. You are charged before generation starts, and if it fails or produces no result the charge is refunded automatically.`,
    };
  }
  ...
  const retryTip = NO_RETRY_DISCOUNT_TOOLS.has(tool.internalId)
    ? ""
    : " Tip: instantly re-running this tool on the same file is half price.";
  return {
    title: `Use ${tool.displayName}?`,
    message: `${costText} You will be charged before generation starts. If generation fails or no result is produced, the charge is refunded automatically.${retryTip}`,
  };
}
...
export async function requestAiSpendConfirmation(meta?: AiRequestMeta) {
  detectConsecutiveRetry(meta);
  const context = getAiSpendContext(meta);
  if (!context) return true;
  if (consumeAiSpendApproval(meta)) {
    rememberPaidToolRun(meta);
    return true;
  }
...
export async function enhancePhoto(
  ...
  instruction: string,
  logoUrl?: string,
  options?: { imageSize?: "1K" | "2K"; aspectRatio?: string; promptEditRequest?: string; outfitRequest?: string; resizeFormat?: string; sceneMode?: "extend" | "replace"; backgroundRequest?: string; toolId?: string; tier?: string; retryOfJobId?: string }
): Promise<string> {
