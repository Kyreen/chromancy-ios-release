...
const bundlePricing: Record<UnlockCategory, BundlePricingPreview> = {
  photo: {
    ...
    displayPrice: "12 / 20 / 39 credits",
    summary: "Photo AI pricing varies by tool and follows the final wallet tiers.",
    note: "Wallet top-ups add 50 / 100 / 200 / 500 credits.",
  },
  video: {
    zar: 0,
    uses: 0,
  ...
  business: {
    ...
    displayPrice: "20 / 39 credits",
    summary: "Level Up tools use the final AI credit tiers depending on the tool. CREATE costs 50 credits.",
    note: "Wallet top-ups add 50 / 100 / 200 / 500 credits.",
  },
};
...
export function getWalletTopUpLabel() {
  // Describe top-ups as the wallet CREDITS you receive, not as a price — the store
  // shows the actual localized purchase price (App Review 2.1(b)).
  return `${walletTopUpPricing.map((amount) => `${amount}`).join(" / ")} credits`;
}
...
export function formatZarUsd(zar: number, _exchangeRate?: number) {
  // Wallet amounts are displayed as credits (1 credit = R1 internally).
  return `${formatZarAmount(zar)} credits`;
}
