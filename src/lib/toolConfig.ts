export const AI_PRICE_TIERS = {
  ...
  // tier5 = premium AI tool (CREATE). No retry discount: retryZar = full.
  tier5: { zar: 50, retryZar: 50 },
  // tier6 = CLOTHES CHANGER: 39 credits, retries 25.
  tier6: { zar: 39, retryZar: 25 },
  // tier7 = RESIZE DESIGN: 39 credits, retries 29.
  tier7: { zar: 39, retryZar: 29 },
} as const;
...
export const TOOL_CONFIGS = [
  ...
  { internalId: "clothes_changer", displayName: "CLOTHES CHANGER", category: "photo", pricingTier: "tier6", isAi: true, trialEligible: false },
  ...
  { internalId: "video_reverse", displayName: "VIDEO REVERSE", category: "video", pricingTier: "free", isAi: false, trialEligible: false },
  ...
  { internalId: "resize_design", displayName: "RESIZE DESIGN", category: "design", pricingTier: "tier7", isAi: true, trialEligible: false },
  ...
  { internalId: "create", displayName: "CREATE", category: "business", pricingTier: "tier5", isAi: true, trialEligible: false, proCreditCost: 3 },
...
// A discounted retry never drops below this floor, so the cheapest tiers still
// clear the AI + app-store costs and stay profitable. Bump to 10 for more margin
// (tier2+ retries already sit at/above this, so only tier1 is affected).
export const RETRY_PRICE_FLOOR_ZAR = 8;
...
export function getToolPriceZar(internalId?: string | null, retry = false) {
  ...
  if (!retry) return price.zar;
  // Apply the retry discount, but never below the floor and never above full price.
  return Math.min(price.zar, Math.max(price.retryZar, RETRY_PRICE_FLOOR_ZAR));
}
