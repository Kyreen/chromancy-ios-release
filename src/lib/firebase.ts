...
// On iOS, GIDSignIn requires an iOSClientId. If a dedicated iOS OAuth client hasn't been
// created in the Firebase console, the web client ID works correctly for native sign-in
// as long as its reversed form is registered as a CFBundleURLScheme in Info.plist
// (which it is: com.googleusercontent.apps.51608421110-r384r8dnhr50kcqcrg1ptu80kjitp45c).
const GOOGLE_IOS_CLIENT_ID = (import.meta.env.VITE_GOOGLE_IOS_CLIENT_ID || GOOGLE_WEB_CLIENT_ID).trim();
...
export function isNativeGoogleLoginConfigured() {
  ...
  // GOOGLE_IOS_CLIENT_ID now falls back to GOOGLE_WEB_CLIENT_ID, so this is always
  // true when Firebase is configured at all.
...
async function ensureNativeAppleLoginInitialized() {
  ...
  nativeAppleInitPromise = (async () => {
    ...
    // Do NOT pass clientId for native iOS — @capgo/capacitor-social-login v8 uses
    // ASAuthorizationAppleIDProvider natively and resolves the bundle ID itself.
    // Passing clientId here triggers the web-based flow which requires a redirect URL
    // and causes the "not handled" / "needs to log in on their phone" error.
    await SocialLogin.initialize({
      apple: {
