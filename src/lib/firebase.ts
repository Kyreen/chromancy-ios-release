...
export function isNativeGoogleLoginConfigured() {
  ...
  return Boolean(GOOGLE_WEB_CLIENT_ID && GOOGLE_IOS_CLIENT_ID && GOOGLE_IOS_CLIENT_ID !== GOOGLE_WEB_CLIENT_ID);
}
...
async function ensureNativeGoogleLoginInitialized() {
  ...
  nativeGoogleInitPromise = (async () => {
    ...
    if (isIos && (!GOOGLE_IOS_CLIENT_ID || GOOGLE_IOS_CLIENT_ID === GOOGLE_WEB_CLIENT_ID)) {
...
async function getNativeAppleCredential() {
  ...
  // Apple shares the user's name only in this authorization response (never inside the
  // ID token) and only on the FIRST sign-in — capture it here so the app can populate
  // the profile automatically and never has to ask the user to type their name
  // (App Review Guideline 4: obtain the name from Sign in with Apple).
  const profile = (providerResult as any)?.profile || (providerResult as any) || {};
  ...
  const appleFullName = [givenName, familyName].filter(Boolean).join(" ").trim();

  // nonce is optional but improves security if the plugin returns one
  const nonce = (providerResult as any)?.nonce ?? undefined;
  return { credential: appleProvider.credential({ idToken, rawNonce: nonce }), appleFullName };
}
...
async function ensureUserProfile(user: FirebaseUser) {
  try {
    ...
    if (displayName) baseProfile.displayName = displayName;
    ...
    if (displayName && !(existingProfile as any)?.displayName) syncPatch.displayName = displayName;
...
export async function loginWithApple() {
  ...
  if (isNativeApp) {
    try {
      const { credential, appleFullName } = await getNativeAppleCredential();
      const result = await signInWithCredential(auth, credential);

      // Populate the profile name from the Sign in with Apple credential so the user
      // is never asked to type it (Apple provides it only on the first authorization).
      if (appleFullName && !result.user.displayName) {
        try {
          await updateProfile(result.user, { displayName: appleFullName });
        } catch (profileError) {
          console.warn("Could not apply Apple-provided name to the profile", profileError);
        }
      }

      await ensureUserProfile({
        ...result.user,
        displayName: result.user.displayName || appleFullName || null,
      } as FirebaseUser);
      return result.user;
    } catch (error) {
      if (!isUserCancelledAuth(error)) {
