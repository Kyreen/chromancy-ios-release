...
// The wallet is displayed as CREDITS everywhere (1 credit = R1 internally — the
// backend still accounts in cents; this is a display-label change only). Real money
// only ever appears as the store's localized purchase price (e.g. "$2.99") pulled
// from StoreKit / Google Play. The legacy `currency` parameter is accepted so older
// call sites keep compiling, but it no longer changes the output.
export function formatMoney(zar: number, _currency: Currency = getCurrency()): string {
  const amount = Number(zar) || 0;
  const rendered = Number.isInteger(amount) ? `${amount}` : amount.toFixed(2);
  return `${rendered} ${Math.abs(amount) === 1 ? "credit" : "credits"}`;
}
