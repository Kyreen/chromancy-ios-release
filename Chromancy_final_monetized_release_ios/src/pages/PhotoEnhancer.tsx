...
import { cancelActiveAiRequests, enhancePhoto, generateVideo, posePerfect, isUnsuitableInputError, getAiRequestErrorMessage, isAiGenerationCancelledError, requestAiSpendConfirmation } from "../lib/gemini";
...
const PREMIUM_TOOLS = [
  ...
  { id: "animate", icon: Play, label: "ANIMATE", instruction: "Make this photo animation dramatically stronger with higher realism, better movement quality, smoother consistency, and natural motion while preserving the subject's exact identity, facial structure, and body proportions.", isVideo: true, description: "Add motion" },
];
...
export function PhotoEnhancer({ tier, onNavigate }: PhotoEnhancerProps) {
  ...
  const getUnsuitableInputMessage = (toolId: string) => {
    switch (toolId) {
      case "animate":
        return "This photo is not suited for Animate. Please upload a different photo with a clearer subject or more natural motion.";
      case "outpaint":
  ...
  const handleEnhance = async (toolId?: string, sourceOverride?: string) => {
    ...
    try {
      ...
      if (id === "outpaint") {
        ...
        result = await generateVideo(sourceImage, `${tool.instruction} ${PHOTO_AI_FACE_PRESERVATION_RULE} Output should feel high quality, realistic, detailed, and natural.`, aiMeta);
      } else if (id === "hd") {
        result = await enhancePhoto(
          sourceImage,
