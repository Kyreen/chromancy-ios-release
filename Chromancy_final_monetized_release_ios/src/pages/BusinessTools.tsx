...
import { cancelActiveAiRequests, createBusinessGraphic, createBusinessGraphicDocument, enhancePhoto, getAiRequestErrorMessage, isAiGenerationCancelledError, predictPerformance, requestAiSpendConfirmation } from "../lib/gemini";
      ...
      switch (toolId) {
        case "mockup":
          result = await enhancePhoto(
            file!,
            `Create a realistic premium mockup using this uploaded design or visual. Mockup request: ${mockupPrompt || "professional billboard, poster, packaging, product shot, storefront, or branded scene"}. Preserve the original uploaded artwork or subject faithfully while placing it into a believable real-world scene with precise perspective, polished materials, strong commercial lighting, clean shadows, print realism, and an expensive professional presentation. Do not add extra words, logos, labels, app names, watermarks, people, products, or unrelated props that were not supplied by the user or already present in the upload.`,
            undefined,
            aiMeta,
          );
          setCurrentExportTarget(null);
        ...
        case "food":
          result = await enhancePhoto(
            file!,
            "Make this food photo look like it was shot by a professional food photographer for a premium restaurant campaign. Greatly improve appetite appeal, natural color, steam/freshness cues where already plausible, plating polish, texture detail, sauce/gloss highlights, clean shadow depth, balanced exposure, editorial contrast, and crisp commercial finish. Preserve the actual food, plate, ingredients, camera angle, composition, and scene identity. Do not drastically change the dish, do not invent different food, and do not add text, logos, labels, app names, watermarks, or signatures.",
            undefined,
            aiMeta,
          );
          setCurrentExportTarget(null);
        ...
        case "studio":
          result = await enhancePhoto(
            file!,
            "Turn this uploaded product image into a polished professional studio shot with premium commercial lighting, clean shadows, natural reflections, crisp product detail, balanced tones, controlled background, and an expensive advertising finish that feels brand-ready and premium. Preserve the exact product, subject shape, logo/packaging details, proportions, colors, and identity. Do not replace the product, do not redesign it, and do not add text, labels, app names, watermarks, or signatures.",
            undefined,
            aiMeta,
          );
          setCurrentExportTarget(null);
        ...
        case "pro":
          result = await enhancePhoto(
            file!,
            `Brand this uploaded photo at a premium commercial level without turning it into a separate poster. Keep the original photo as the main image and preserve the subject, crop, scene, face, body, product details, and photo identity exactly. ${logo ? "Integrate the provided logo as a polished brand mark, not a basic sticker." : "No logo was provided, so do not invent a logo or brand mark."} ${proText.trim() ? `Add only this exact user-provided text: "${proText.trim()}".` : "No text was provided, so do not add any words or slogans."} Use a refined luxury lower-third, soft lower fade, translucent editorial panel, elegant corner lockup, or tasteful gradient brand band only if it improves the photo. Use premium typography only for user-provided text, strong hierarchy, disciplined margins, clean alignment, refined shadows, and commercial social-ad polish. Never add app names, watermarks, signatures, filler text, fake contact details, or any text the user did not provide.`,
            logo || undefined,
            aiMeta,
          );
          setCurrentExportTarget(null);
        ...
        case "create": {
          ...
          try {
            const documentResult = await createBusinessGraphicDocument({
            ...
            if (!documentResult.previewImageUrl) {
              throw new Error("CREATE preview missing.");
            }
            result = documentResult.previewImageUrl;
          } catch (planError) {
            console.error("CREATE design document generation failed, falling back to direct premium render", planError);
