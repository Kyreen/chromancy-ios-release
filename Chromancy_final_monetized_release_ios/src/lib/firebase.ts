...
import { browserLocalPersistence, getAuth, GoogleAuthProvider, signInWithPopup, sendPasswordResetEmail, setPersistence, signOut, deleteUser, signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile, User as FirebaseUser, EmailAuthProvider, reauthenticateWithCredential, reauthenticateWithPopup, signInWithCredential } from "firebase/auth";
...
async function ensureSocialLoginInitialized() {
  ...
  socialLoginInitPromise = (async () => {
    ...
    if (isIos && !GOOGLE_IOS_CLIENT_ID) {
      throw new Error('Google sign-in on iOS is missing VITE_GOOGLE_IOS_CLIENT_ID. Add the iOS OAuth client ID before building the iPhone app.');
    }
...
export async function loginWithGoogle() {
  ...
  await setPersistence(auth, browserLocalPersistence).catch((error) => {
    console.warn("Could not set Firebase auth persistence before Google sign-in", error);
  });
...
async function purgeServerAccountData(uid: string) {
...
async function deleteServerAccount(uid: string) {
...
async function deleteFirebaseAuthAccount(user: FirebaseUser) {
