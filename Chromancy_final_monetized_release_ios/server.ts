...
const GEMINI_AI_CONFIGURED = Boolean(process.env.GEMINI_API_KEY);
...
async function createAiHold(req: express.Request, res: express.Response, toolId: string, options: { tier?: UserTier; retryOfJobId?: string | null } = {}) {
  ...
  const holdContext = await mutateUsageStore((store) => {
    ...
    const amountCents = zarToCents(getToolPriceZar(tool.internalId, isRetry));
    ...
    if (!options.retryOfJobId && tool.trialEligible && currentTrialCount < FREE_TRIAL_LIMIT) {
      source = "trial";
    } else if ((subscriberTier === "pro" || subscriberTier === "premium") && (account.proCredits || 0) >= proCreditCost) {
      source = "pro_credit";
...
async function grantWalletTopUp(uid: string, productId: string, purchaseId: string) {
...
async function reverseWalletTopUp(uid: string, productId: string, purchaseId: string, eventId: string) {
  const topUp = WALLET_TOP_UP_BY_PRODUCT[productId];
  if (!topUp) return { ok: false, reason: "UNKNOWN_PRODUCT" };
  ...
  const result = await mutateUsageStore((store) => {
    ...
    if (account.reversedPurchases[purchaseId]) {
      return { ok: true, duplicate: true, snapshot: buildSnapshot(store, accountKey, null, hashValue("webhook")), account };
    }

    const amountCents = zarToCents(topUp.zar);
    account.walletCents = Math.round(Number(account.walletCents) || 0) - amountCents;
    account.reversedPurchases[purchaseId] = eventId;
    addLedgerTransaction(account, {
      type: "refund",
...
async function startServer() {
  ...
  app.get("/api/health", (_req, res) => {
    res.json({
      ...
      aiConfigured: GEMINI_AI_CONFIGURED,
    });
  });
  ...
  app.post("/api/billing/revenuecat-webhook", async (req, res) => {
    ...
    if (event.type === "CANCELLATION") {
      const reversed = await reverseWalletTopUp(event.appUserId, event.productId, event.transactionId, event.id);
    ...
    const topUp = await grantWalletTopUp(event.appUserId, event.productId, event.transactionId);
