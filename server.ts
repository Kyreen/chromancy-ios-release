...
// Keep the per-account ledger tight: the whole usage store lives in ONE Firestore
// document, and Firestore caps both index entries (~40k) and document size (1 MiB).
// The wallet UI shows at most the last 100 transactions.
...
// Strip zero counters and empty records before persisting. The whole store is ONE
// Firestore document, which hard-caps index entries (~40k) and size (1 MiB) — storing
// hundreds of all-zero trial counters (32 tool keys per device/account) blew past the
// index limit and made every write fail with INVALID_ARGUMENT ("too many index
// entries"), which surfaced as 503s on wallet/usage endpoints. Reads are safe because
// every read path re-normalizes (normalizeToolTrials / normalizeTrialCounts /
// normalizeRecordMap refill missing keys with zeros).
...
function compactUsageStore(store: UsageStore): UsageStore {
  const compacted: any = JSON.parse(JSON.stringify(store));

  for (const [key, account] of Object.entries<any>(compacted.accounts || {})) {
    account.freeTrials = compactCountMap(account.freeTrials);
    account.toolFreeTrials = compactCountMap(account.toolFreeTrials);
    account.credits = compactCountMap(account.credits);
    account.premiumUsage = compactCountMap(account.premiumUsage);
    for (const mapKey of ["freeTrials", "toolFreeTrials", "credits", "premiumUsage"]) {
      if (account[mapKey] === undefined) delete account[mapKey];
    }
    // Drop empty containers; read paths recreate them on demand.
    for (const mapKey of ["holds", "jobs", "processedPurchases", "processedPurchaseFingerprints", "processedWebhookEvents", "reversedPurchases"]) {
      if (account[mapKey] && typeof account[mapKey] === "object" && !Object.keys(account[mapKey]).length) delete account[mapKey];
    }
    if (Array.isArray(account.transactions)) {
      account.transactions = account.transactions.slice(-LEDGER_LIMIT);
      if (!account.transactions.length) delete account.transactions;
    }
    void key;
  }

  const devices: Record<string, any> = {};
  for (const [deviceKey, device] of Object.entries<any>(compacted.devices || {})) {
    const freeTrials = compactCountMap(device?.freeTrials);
    const toolFreeTrials = compactCountMap(device?.toolFreeTrials);
    if (!freeTrials && !toolFreeTrials) continue; // never used a trial — identical to absent
    devices[deviceKey] = {
      ...(freeTrials ? { freeTrials } : {}),
      ...(toolFreeTrials ? { toolFreeTrials } : {}),
    };
  }
  compacted.devices = devices;

  const ipTrials: Record<string, any> = {};
  for (const [ipKey, trials] of Object.entries<any>(compacted.ipTrials || {})) {
    const kept = compactCountMap(trials);
    if (kept) ipTrials[ipKey] = kept;
  }
  compacted.ipTrials = ipTrials;

  const ipToolTrials: Record<string, any> = {};
  for (const [ipKey, trials] of Object.entries<any>(compacted.ipToolTrials || {})) {
    const kept = compactCountMap(trials);
    if (kept) ipToolTrials[ipKey] = kept;
  }
  compacted.ipToolTrials = ipToolTrials;

  return compacted as UsageStore;
}

function serialiseUsageStore(store: UsageStore): UsageStore {
  return compactUsageStore(JSON.parse(JSON.stringify(normalizeUsageStore(store))) as UsageStore);
}
...
async function createAiHold(req: express.Request, res: express.Response, toolId: string, options: { tier?: UserTier; retryOfJobId?: string | null } = {}) {
  ...
  const holdContext = await mutateUsageStore((store) => {
    ...
    if ((subscriberTier === "pro" || subscriberTier === "premium") && (account.proCredits || 0) >= proCreditCost) {
      ...
      // Wallet amounts are shown to users as credits (1 credit = R1 internally).
      const entitlementMessage = hasSubscription
        ? `${prefix} This tool needs ${creditText}, but you have ${creditBalance} subscription credit${creditBalance === 1 ? "" : "s"} left and ${walletBalance.toFixed(2)} wallet credits${walletDebt > 0 ? `, with ${walletDebt.toFixed(2)} credits still owed from a refunded top-up` : ""}. Please top up your wallet to continue.`
        : `${prefix} This tool costs ${priceZar.toFixed(2)} credits or ${creditText}. Please top up your wallet or subscribe to continue.`;
      return {
        errorStatus: 402,
...
async function grantWalletTopUp(uid: string, productId: string, purchaseId: string, purchasedAt?: string | null, eventId?: string | null) {
  ...
  const result = await mutateUsageStore((store) => {
    ...
    // Credit each RevenueCat purchase exactly once. Guard on the store transaction id, on the
    // unique RevenueCat event id (catches webhook retries with the same event), and on a
    // product+time fingerprint (catches redeliveries with a fresh transaction id). Any one
    // match means it was already credited — never add funds twice.
    ...
    if (eventId && account.processedWebhookEvents[eventId]) {
    ...
    // CROSS-ACCOUNT GUARD: one store transaction must never credit two different app
    // accounts. RevenueCat re-attaches the store receipt (with its full purchase history)
    // when a new app account buys on the same Apple/Google ID, so a transaction already
    // credited to ANY account is a duplicate here — record it and add nothing.
    const creditedToAnotherAccount = Object.entries(store.accounts || {}).some(([otherKey, otherAccount]) => {
      if (otherKey === accountKey || !otherAccount) return false;
      const otherPurchases = normalizeRecordMap<string>((otherAccount as AccountRecord).processedPurchases);
      const otherFingerprints = normalizeRecordMap<string>((otherAccount as AccountRecord).processedPurchaseFingerprints);
      return Boolean(otherPurchases[purchaseId] || (purchaseFingerprint && otherFingerprints[purchaseFingerprint]));
    });
    if (creditedToAnotherAccount) {
      account.processedPurchases[purchaseId] = productId;
      if (purchaseFingerprint) {
        account.processedPurchaseFingerprints[purchaseFingerprint] = purchaseId;
      }
      if (eventId) {
        account.processedWebhookEvents[eventId] = `wallet_duplicate:${productId}`;
      }
      return { ok: true, duplicate: true, snapshot: buildSnapshot(store, accountKey, null, hashValue("webhook")), account };
    }
    ...
    if (eventId) {
      account.processedWebhookEvents[eventId] = `wallet:${productId}`;
    }
    addLedgerTransaction(account, {
      type: "top-up",
      amountCents,
...
function getRevenueCatEventPayload(body: any) {
  ...
  // Deterministic fallback (never random): a random id would defeat idempotency and let the
  // same purchase be credited twice. Same user+product+time always yields the same key.
  const transactionId = event.transaction_id
    || event.original_transaction_id
    || event.id
    || `rc_synth:${typeof appUserId === "string" ? appUserId : ""}:${productId || ""}:${event.purchased_at_ms || event.purchased_at || event.event_timestamp_ms || ""}`;
  const type = String(event.type || event.event_type || "").toUpperCase();
  const toIdList = (value: unknown) => Array.isArray(value)
  ...
  return {
    ...
    // TRANSFER events: RevenueCat moves purchases between app user ids (e.g. a user
    // buys, then signs in and gets a new Firebase UID). Wallet credits must follow.
    transferredFrom: toIdList(event.transferred_from),
    transferredTo: toIdList(event.transferred_to),
  };
}

// When RevenueCat transfers purchases to a new app user id, move the wallet balance
// with them — otherwise credits strand on the old (often pre-login) account and the
// user's wallet shows zero after signing in. Idempotent per webhook event id.
async function transferWalletBalance(fromUid: string, toUid: string, eventId: string) {
  const fromKey = getAccountKey(fromUid)!;
  const toKey = getAccountKey(toUid)!;
  return await mutateUsageStore((store) => {
    const fromAccount = getOrCreateAccountRecord(store, fromKey);
    const toAccount = getOrCreateAccountRecord(store, toKey);
    toAccount.processedWebhookEvents = normalizeRecordMap<string>(toAccount.processedWebhookEvents);
    if (toAccount.processedWebhookEvents[eventId]) {
      return { moved: 0, duplicate: true };
    }

    const movableCents = Math.max(0, Math.round(Number(fromAccount.walletCents) || 0));
    toAccount.processedWebhookEvents[eventId] = `transfer:${fromUid}`;
    if (movableCents <= 0) {
      return { moved: 0, duplicate: false };
    }

    fromAccount.walletCents = Math.round(Number(fromAccount.walletCents) || 0) - movableCents;
    toAccount.walletCents = Math.round(Number(toAccount.walletCents) || 0) + movableCents;
    addLedgerTransaction(fromAccount, {
      type: "usage",
      amountCents: -movableCents,
      note: "Wallet credits moved to your signed-in account",
    });
    addLedgerTransaction(toAccount, {
      type: "top-up",
      amountCents: movableCents,
      note: "Wallet credits transferred from your previous session",
    });
    return { moved: movableCents, duplicate: false };
  });
}
...
async function syncCustomerInfoPurchases(uid: string, body: any) {
  ...
  if (syncReason === "wallet_top_up") {
    ...
    // SECURITY: customerInfo lists the store account's ENTIRE lifetime consumable history,
    // not just the purchase that triggered this sync. Only credit transactions completed
    // within the last few minutes. Without this window, a brand-new app account's first
    // top-up re-credits every historical purchase ever made on the same Apple/Google ID
    // (observed: a R100 purchase crediting R700). Old transactions were already credited
    // when they originally happened — the webhook and the fresh window cover the new one.
    const FRESH_PURCHASE_WINDOW_MS = 15 * 60_000;
    for (const [index, transaction] of transactions.entries()) {
      ...
      const purchaseMs = toMillis(purchaseDate || null);
      if (!purchaseMs || Math.abs(Date.now() - purchaseMs) > FRESH_PURCHASE_WINDOW_MS) continue;
...
async function startServer() {
  ...
  app.post("/api/billing/revenuecat-webhook", async (req, res) => {
    ...
    // RevenueCat TRANSFER: purchases moved to another app user id (e.g. purchase made
    // before signing in, then the user signs in and gets a Firebase UID). Move the
    // wallet balance along so credits don't strand on the old account.
    if (event.type === "TRANSFER") {
      const fromUid = event.transferredFrom.find((value) => !isAnonymousRevenueCatId(value)) || event.transferredFrom[0];
      const toUid = event.transferredTo.find((value) => !isAnonymousRevenueCatId(value)) || event.transferredTo[0];
      if (!fromUid || !toUid || fromUid === toUid) {
        return res.json({ ok: true, ignored: "transfer_unactionable" });
      }
      const result = await transferWalletBalance(fromUid, toUid, event.id);
      return res.json({ ok: true, handled: "wallet_transfer", movedCents: result.moved, duplicate: result.duplicate });
    }
    ...
    const topUp = await grantWalletTopUp(event.appUserId, event.productId, event.transactionId, event.purchasedAt || event.eventTimestampAt, event.id);
